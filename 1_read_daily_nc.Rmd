...
csv_from_nc_data = function(daily_nc_directory = "F:/CTM_daily_nc",
                            years_to_take = c("2010"),
                            months_to_take = c("01"),
                            area_csv_file_path = "C:/Users/ellio/OneDrive/Desktop/cleaned_version/area_data/area.csv",
                            only_land = TRUE){
  
    # example inputs
    daily_nc_directory = "F:/CTM_daily_nc"
    years_to_take = c("2010")
    months_to_take = c("01", "02", "03", "04", "05", "06", "07", "08", "09", "10", "11", "12")
    area_csv_file_path = "C:/Users/ellio/OneDrive/Desktop/cleaned_version/area_data/area.csv"
    calculate_importance = F
...
imported_data = csv_from_nc_data(daily_nc_directory = "F:/CTM_daily_nc",
                                years_to_take = c("2010"),
                                months_to_take = c("01", "02", "03", "04", "05", "06", "07", "08", "09", "10", "11", "12"),
                                area_csv_file_path = "C:/Users/ellio/OneDrive/Desktop/cleaned_version/area_data/area.csv",
                                only_land = FALSE)
...
visualization_df$residuals_using_CTM = visualization_df_using_CTM$residuals

save(standardized_data, interpretable_data, data_to_train, data_to_predict, data_to_analyze_after_prediction, BRF_model, results_dataset_using_Satellite, results_dataset_using_CTM, visualization_df, file = "results_7_31_withsea.RData")
# load("results_7_25.Rdata")

# load("results_7_23.Rdata")

# load("results_7_31.Rdata")
...
visualization_df = visualization_df %>%
  mutate(residuals = Posterior_Estimates - ML_Monthly_Averages,
         # troublepts = ifelse(Posterior_Estimates < 0.004 & ML_Monthly_Averages > 0.007, "red", "black"), # for finding trouble points
         residuals_CTM = Posterior_Estimates - ML_Monthly_Averages_using_CTM)
...
```{r analyzing distribution of troublepoints}

# EDA plots from: the old laptop! named "debugging <something>"
...
heatmap_df = visualization_df %>% 
  group_by(LON, LAT) %>% 
  summarize(average_residuals = mean(residuals),
            average_residuals_CTM = mean(residuals_CTM),
            count = mean(count))
...
NOx_tracer = ncvar_get(gctm_nc_obj, "IJ-AVG-S__NOx", count = c(-1,-1,2)) # retrieves the variable and its SURFACE data
...
```{r heatmaps OLD}
# idea: copy an .nc file, edit it with my data, then re-write it again with different names

gctm_nc_file = "heatmap_palette.nc"
gctm_nc_obj = nc_open(gctm_nc_file, write =TRUE)
our_dim = gctm_nc_obj$dim
our_vars = gctm_nc_obj$var
LON = ncdim_def("Lon-000", units = "", vals = (1:144), create_dimvar = FALSE)
LON_var = ncvar_def(name = "LON", units = "Degrees", dim = our_dim$`Lon-000`, longname = "Longitude")
LAT = ncdim_def("Lat-000", units = "", vals = (1:91), create_dimvar = FALSE)
LAT_var = ncvar_def(name = "LAT", units = "Degrees", dim = our_dim$`Lat-000`, longname = "Latitude")
ALT = ncdim_def('Alt-000', units = "", vals = 1:4, create_dimvar = FALSE)

new_var = ncvar_def(name = "myvar", units = "", dim = list(our_dim$`Lon-000`,our_dim$`Lat-000`,our_dim$`Alt-000`))
# new_nc = nc_create("heatmap_nc_file2.nc", vars = list(LON_var,LAT_var,new_var)) # hopefully will work! Check by putting in crazy data!
new_nc = nc_create("heatmap_nc_file2.nc", vars = list(our_vars$LON, our_vars$LAT, our_vars$`IJ-AVG-S__NOx`), force_v4 = F, verbose = T) # hopefully will work! Check by putting in crazy data!
nc_close(new_nc)

LON = rep(ncvar_get(gctm_nc_obj, "LON"), gctm_nc_obj$var[["LAT"]]$varsize)
LAT = rep(ncvar_get(gctm_nc_obj, "LAT"), each = gctm_nc_obj$var[["LON"]]$varsize)
orig_CTM_conc = our_var[,,1]
orig_CTM_conc_vec = c(orig_CTM_conc)
orig_sat_conc = our_var[,,2]
orig_sat_conc_vec = c(orig_sat_conc)
...
# mydata = c(array1, array2)
mydata_array1 = combined_df$orig_CTM_conc_vec
mydata_array2 = combined_df$orig_sat_conc_vec
mydata_array3 = combined_df$average_residuals
mydata_array4 = combined_df$count
total_data = c(mydata_array1, mydata_array2, mydata_array3, mydata_array4)
# ncvar_put(new_nc, new_var, total_data)
# nc_redef(gctm_nc_obj)

ncvar_put(gctm_nc_obj, our_vars$`IJ-AVG-S__NOx`$name, total_data)

print(c(min(combined_df$average_residuals), max(combined_df$average_residuals)))
print(c(min(combined_df$count), max(combined_df$count)))


# Question: can we replicate...
image(our_var[,,1]) # this one, which should be CTM concentrations
image(our_var[,,2]) # and this one, which should be satellite concentrations

nc_close(new_nc)
# should be exactly like image... just as you would expect
